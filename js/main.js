...
if (disqus_target && related_target) {
	...
	function loadDisqus(target) {
		...
		// Load script asynchronously only when the target hasn't been filled
		if (target && is_disqus_empty) {
	...
	function loadRelatedPosts(target) {
		...
		related_embed.onerror = function() { throw(new Error('Failed to load ' + this.src)); }
...
/*
  	var elems = document.querySelectorAll('.tooltipped:not(.slowtip)');
  	var instances = M.Tooltip.init(elems, {});
    var tooltipelems = document.querySelectorAll('.tooltipped.slowtip');
    var tooltipinstances = M.Tooltip.init(tooltipelems, { "enterDelay" : "500" });

*/
// End initialize tooltips

// Start initialize image fullscreens
...
// End initialize image fullscreens
...
// Start initialize modal popups
	...
	var searchelem = document.querySelectorAll('#searchbar-modal');
	var searchinstance = M.Modal.getInstance(elem);*/
/*	
	var elem = document.querySelector("#searchbar-modal");
    var instance = M.Modal.init(elem, { onOpenEnd : function() {
		$('#search-text').focus();
	}});
	
	elem = document.querySelector("#email-subscribe-modal");
    instance = M.Modal.init(elem, { onOpenEnd : function() {
		$('#email-subscribe-text').focus();
	}});
	
	$('#email-subscribe-form-modal').submit(function() {
		instance.close();
	});
	
	var elem2 = document.getElementById('search-only-nav-action');
	elem2.classList.add('search-loaded');
*/
...
document.querySelectorAll('label[for="menu-toggle"]').forEach(label => {
...
// Begin animate sidenav toggle button
...
! function() {
    ...
    t.prototype.getSize = function() {
		...
		var menuToggle = document.querySelectorAll('label[for="menu-toggle"]');
		Array.prototype.forEach.call(menuToggle, function(label) {
			label.addEventListener(t.animation.event, function() {
				t.status.animating || (t.status.animating = !0, window.requestAnimationFrame(t.triggerAnimation.bind(t)))
			});
		})
...
// End animate sidenav toggle button
...
// Start initializing modal popups. Temporarily copying over all related code from framework file to prevent having to load the entire library that is many times larger
...
/* End anime definition */



/* Start Modal initialization code */
...
// Start initialize modal popups
	...
	var elem = document.getElementById("searchbar-modal");
    var instance = M.Modal.init(elem, { onOpenEnd : function() {
		// $('#search-text').focus();
		document.getElementById("search-text").focus();
	}});
	
	elem = document.getElementById("email-subscribe-modal");
    instance = M.Modal.init(elem, { onOpenEnd : function() {
		// $('#email-subscribe-text').focus();
		document.getElementById("email-subscribe-text").focus();
	}});
	
	/*$('#email-subscribe-form-modal').submit(function() {
		instance.close();
	});*/
	document.getElementById("email-subscribe-form-modal").addEventListener("submit", function() {
		instance.close();
	});
